...
public class Db {
	...
	static {
		...
		try {
			...
			IniReader reader = IniReader.getInstance(1);
	...
	public static long executeUpdate(String sql, Object[] parm , int server){
		...
		try {
            ...
            if (Constants.SHOW_SQL) {
                ...
                Logs.getinfoLogger().info(targetSql);
            }
        ...
        try {
            ...
            for (int i = 0; i < paramNum; i++) {
                if (params[i] instanceof Date) {
                    returnSQL.append(subSQL[i]).append(" '").append(DateUtil.dateUtil2String((Date)params[i], "yyyy-MM-dd HH:mm:ss")).append("' ");
                } else {
